...
from acestep.streaming.families import DEFAULT_FAMILY, get_family, make_backend
...
class StreamingSession:
    ...
    def create(
        ...
        *,
        ...
        """
        # Every family owns its create path (FamilySpec.create_session):
        # TRT profiles, model load, conditioning encode and the rest are
        # family decisions, so nothing family-shaped lives here. ACE's
        # creator is acestep.streaming.ace_session; SA3's is
        # acestep.streaming.sa3_session. An unknown family fails loudly
        # at create, never with a silent fallback.
        family = getattr(config, "backend", DEFAULT_FAMILY) or DEFAULT_FAMILY
        spec = get_family(family)
        if spec.create_session is None:
            raise ValueError(
                f"backend family {family!r} declares no create_session"
            )
        return spec.create_session(
            cls,
            audio=audio,
            config=config,
